...
PythonMachine LearningOptimizationStatus
...
R 
2
  Skoru (Açıklayıcılık Oranı)	Mean Absolute Error (MAE)	Tahmin Doğruluğu (Log-scale)*
...
*Tahmin Doğruluğu, modelin logaritmik ölçekteki tahmin hatasından (MAPE) arındırılmış net doğruluğunu (
100
−
MAPE
100−MAPE) temsil eder.
...
platform_YouTube (Platformun YouTube olması)
InternetUsers_PctOfPopulationUsingInternet (Ülkedeki internet kullanım yüzdesi)
reach_index (Türettiğimiz Erişim Endeksi değişkeni)
...
Strateji 1 (Sabit Oranlı - %40 / %35 / %25): Harcanan: 
83
,
332
∣
...
n
:
83,332∣Kalan:16,668 (%16.7 Bütçe İsrafı) | Toplam Beğeni: 3,470,459
Strateji 2 (Esnek Capped - Önerilen): Harcanan: 
99
,
530
∣
...
n
:
99,530∣Kalan:470 (%0.5 Bütçe İsrafı) | Toplam Beğeni: 4,675,339 (Katı dağılıma göre %34 verim artışı!)
Strateji 3 (Sınırsız Global): Harcanan: 
99
,
789
∣
...
n
:
99,789∣Kalan:210 (%0.2 Bütçe İsrafı) | Toplam Beğeni: 6,392,256 (Bütçenin %99'u tek platforma yığıldığı için yüksek riskli.)
💻 İnteraktif Karar Destek Arayüzü (GUI)
Notebook içerisinde yer alan ipywidgets tabanlı canlı simülatör sayesinde:

Hedef pazar sektörü açılır listeden seçilir.
Kampanya bütçesi kaydırma çubuğuyla belirlenir.
"Optimizasyonu Çalıştır" butonuna basıldığında model çalışır ve anlık bütçe analizi, harcanan/kalan para, tahmini toplam beğeni, CPL (Beğeni başına maliyet) ve ROI (Beğeni / $1) metriklerini hesaplar.
Çıktının altında açılan sekmeler içerisindeki HTML tablolarında seçilen tüm influencer'ların detaylı listesi sunulur.
📁 Proje Dizin Yapısı
text
